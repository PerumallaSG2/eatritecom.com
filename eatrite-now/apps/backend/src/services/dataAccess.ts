...
export class DataAccessService {
   ...
   */
  async getCatalogMeals(
    filters: { popular?: boolean; limit?: number; category?: string } = {}
  ) {
    try {
      const pool = getPool()
      let query = `
      ...
      const uniqueMeals = result.recordset.reduce(
        (acc: any[], current: any) => {
  ...
  async getCatalogCategories() {
    try {
      ...
      const uniqueCategories = result.recordset.reduce(
        (acc: any[], current: any) => {
  ...
  async getCatalogPlans() {
    try {
      ...
      const uniquePlans = result.recordset.reduce(
        (acc: any[], current: any) => {
  ...
  async saveUserPlanSubscription(data: {
    ...
    try {
      ...
      const result = await pool
        ...
        .input('endDate', sql.Date, data.endDate || null).query(`
  ...
  async saveUserQuizResponse(data: any) {
    try {
      ...
      const result = await pool
        ...
        .input('weeklyBudget', sql.NVarChar, data.weeklyBudget || null).query(`
  ...
  async saveUserContactMessage(data: {
    ...
    try {
      ...
      const result = await pool
        ...
        .input('message', sql.NVarChar, data.message).query(`
