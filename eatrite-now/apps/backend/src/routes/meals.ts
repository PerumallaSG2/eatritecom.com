...
router.get('/', async (req, res) => {
  try {
    ...
    if (search) {
      query += ' AND (m.name LIKE @search OR m.description LIKE @search OR m.ingredients LIKE @search)';
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // Fall back to mock data if database fails
    const fallbackResult = FallbackDataService.getMealById(parseInt(req.params.id));
