...
interface SmartSuggestion {
  id: string;
  type: 'upsell' | 'cross-sell' | 'bundle' | 'nutrition' | 'popular' | 'seasonal';
  title: string;
  subtitle: string;
  item: CartItem;
  reason: string;
...
const generateSmartSuggestions = (_cartItems: CartItem[]): SmartSuggestion[] => [
  {
...
export const SmartCartOptimization: React.FC = () => {
  ...
  const calculateNutritionGoals = (items: CartItem[]) => {
    ...
    const totalFiber = items.reduce((sum, item) => sum + (item.fiber * item.quantity), 0);
    ...
    const goals: NutritionGoal[] = [
      {
        ...
        target: 1800,
        unit: 'kcal',
        status: totalCalories > 1800 ? 'exceeded' : totalCalories > 1600 ? 'met' : totalCalories > 1400 ? 'close' : 'low'
      },
      {
        name: 'Protein',
        ...
        target: 120,
        unit: 'g',
        status: totalProtein > 120 ? 'exceeded' : totalProtein > 100 ? 'met' : totalProtein > 80 ? 'close' : 'low',
        recommendation: totalProtein < 100 ? 'Consider adding a protein-rich meal' : undefined
      },
      {
        name: 'Carbs',
        current: totalCarbs,
        target: 150,
        unit: 'g',
        status: totalCarbs > 150 ? 'exceeded' : totalCarbs > 120 ? 'met' : totalCarbs > 100 ? 'close' : 'low'
      },
      {
        name: 'Fiber',
        current: totalFiber,
        target: 25,
        unit: 'g',
        status: totalFiber > 25 ? 'exceeded' : totalFiber > 20 ? 'met' : totalFiber > 15 ? 'close' : 'low',
        recommendation: totalFiber < 20 ? 'Add vegetables or whole grains for more fiber' : undefined
      }
    ];
  ...
  const optimizeCart = (items: CartItem[]) => {
    ...
    setTimeout(() => {
      ...
      const totalCurrentPrice = items.reduce((sum, item) => sum + (item.price * item.quantity), 0);
      
      const nutritionScore = Math.min(100, Math.round(
        (nutritionGoals.filter(goal => goal.status === 'met' || goal.status === 'exceeded').length / nutritionGoals.length) * 100
      ));
      
      const varietyScore = Math.min(100, (new Set(items.map(item => item.category)).size / 5) * 100);
      const convenienceScore = Math.round((items.filter(item => item.prepTime.includes('2')).length / items.length) * 100);
  ...
  const getSuggestionIcon = (type: string) => {
    switch (type) {
      ...
      case 'upsell': return Zap;
      case 'cross-sell': return Heart;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-100">
                AI-powered suggestions to optimize nutrition, savings, and satisfaction
              </p>
            </div>
                      ...
                      <div key={item.id} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4 text-sm text-gray-600 mt-1">
                            ...
                            <span className="text-blue-600">{item.prepTime}</span>
                          </div>
                ...
                <div className="space-y-4">
                  <StaggeredAnimation>
                    {suggestions.map((suggestion) => {
                      ...
                      return (
                        <div key={suggestion.id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                          <div className="p-4">
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex items-start space-x-3">
                                <div className={`w-10 h-10 bg-gradient-to-r ${
                                  suggestion.type === 'bundle' ? 'from-green-500 to-emerald-500' :
                                  suggestion.type === 'nutrition' ? 'from-blue-500 to-cyan-500' :
                                  suggestion.type === 'popular' ? 'from-purple-500 to-pink-500' :
                                  suggestion.type === 'upsell' ? 'from-orange-500 to-red-500' :
                              ...
                              <Floating>
                                <div className="bg-gray-50 rounded-lg p-4 mt-3">
                                  <div className="flex items-center space-x-3 mb-3">
                                    <span className="text-2xl">{suggestion.item.image}</span>
                                    <div>
                                      <h5 className="font-semibold text-gray-900">{suggestion.item.name}</h5>
                                      <div className="flex items-center space-x-4 text-sm text-gray-600">
                                        <span>{suggestion.item.calories} cal</span>
                                        <span>{suggestion.item.protein}g protein</span>
                                        <span className="flex items-center">
                                          <Star className="w-3 h-3 text-yellow-500 mr-1" />
                              ...
                              <div className="flex items-center space-x-2">
                                ...
                                {suggestion.item.originalPrice && (
                                  <span className="text-sm text-gray-500 line-through">
                                    {formatCurrency(suggestion.item.originalPrice)}
                                  </span>
                                )}
                              </div>
                ...
                <div className="bg-white rounded-2xl p-6 shadow-lg">
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-4">
                    <div className="text-center p-3 bg-green-50 rounded-lg">
                      <div className="text-xl font-bold text-green-600">{optimization.nutritionScore}%</div>
                      <div className="text-xs text-gray-600">Nutrition</div>
                    </div>
                    <div className="text-center p-3 bg-blue-50 rounded-lg">
                      <div className="text-xl font-bold text-blue-600">{optimization.varietyScore}%</div>
                      <div className="text-xs text-gray-600">Variety</div>
                    </div>
                    <div className="text-center p-3 bg-purple-50 rounded-lg">
                      <div className="text-xl font-bold text-purple-600">{optimization.convenienceScore}%</div>
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm font-semibold text-green-700">
                          You're saving {formatCurrency(optimization.totalSavings)}
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  {optimization.warnings.length > 0 && (
                    <div className="space-y-2">
                      {optimization.warnings.map((warning, index) => (
                        <div key={index} className="flex items-start space-x-2 bg-yellow-50 border border-yellow-200 rounded-lg p-2">
                    ...
                    return (
                      <div key={goal.name} className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full ${getNutritionStatusColor(goal.status)}`}>
                            {goal.status}
                          </span>
                        </div>
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full transition-all duration-1000 ${
                              goal.status === 'exceeded' ? 'bg-purple-500' :
                              goal.status === 'met' ? 'bg-green-500' :
                              goal.status === 'close' ? 'bg-yellow-500' :
                              'bg-red-500'
                            }`}
                        ...
                        {goal.recommendation && (
                          <p className="text-xs text-blue-600 italic">{goal.recommendation}</p>
                        )}
                      </div>
                    );
                  })}
                ...
                <div className="space-y-3">
                  ...
                  {getOriginalTotal() > getSubtotal() && (
                    <div className="flex justify-between text-green-600">
                      <span>Savings</span>
                      <span className="font-semibold">-{formatCurrency(getOriginalTotal() - getSubtotal())}</span>
                    </div>
                  )}
                  ...
                  <div className="border-t border-gray-200 pt-3">
                    ...
                    {getOriginalTotal() > getSubtotal() && (
                      <div className="text-sm text-gray-500">
                        Original: <span className="line-through">{formatCurrency(getOriginalTotal())}</span>
                      </div>
                    )}
                  </div>
