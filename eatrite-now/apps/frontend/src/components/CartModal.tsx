...
const CartModal: React.FC<CartModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      {/* Background overlay - only covers below navbar */}
      <div
        className="fixed inset-0 top-16 bg-[#0F2B1E]/80 backdrop-blur-sm transition-opacity duration-300"
        onClick={onClose}
      />

      {/* Cart sidebar - starts below navbar */}
      <div
        className={`fixed right-0 top-16 h-[calc(100vh-4rem)] w-full max-w-md bg-gradient-to-b from-[#F5F2E8] to-white shadow-2xl transform transition-transform duration-300 overflow-hidden ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="flex flex-col h-full">
          {/* Header */}
          <div className="bg-gradient-to-r from-[#0F2B1E] to-[#0A2418] px-6 pt-6 pb-4 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p
                    className="text-[#D4B46A] text-sm"
                    style={{ fontFamily: 'Inter, sans-serif' }}
                  >
                    {items.length}{' '}
                    {items.length === 1 ? 'delicious meal' : 'delicious meals'}
                  </p>
                </div>
              </div>
              <button
            ...
            {items.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p
                  ...
                  style={{ fontFamily: 'Inter, sans-serif' }}
                >
                  Discover our chef-crafted meals and start your culinary
                  journey!
                </p>
                <button
                  onClick={onClose}
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gradient-to-r from-[#F5EEDC] to-[#F5F2E8] rounded-xl p-6 mt-6 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`font-bold ${deliveryFee === 0 ? 'text-green-600 line-through' : 'text-[#0F2B1E]'}`}
                      style={{ fontFamily: 'Inter, sans-serif' }}
                    >
                      {deliveryFee === 0
                        ? 'FREE'
          ...
          {/* Footer - Always show with debug info */}
          <div className="bg-gradient-to-r from-[#F5EEDC] to-[#F5F2E8] px-6 py-4 border-t border-[#D4B46A]/30 flex-shrink-0">
            {/* Debug info */}
            <div className="mb-4 p-3 bg-yellow-100 border border-yellow-300 rounded-lg text-sm">
              ...
              <p className="text-yellow-700">Items in cart: {items.length}</p>
              <p className="text-yellow-700">Total: ${totalPrice.toFixed(2)}</p>
            ...
            <div className="flex flex-col gap-3">
              {items.length > 0 && (
                <button
                  ...
                  className="w-full bg-white hover:bg-gray-50 text-[#0F2B1E] font-semibold px-6 py-3 rounded-full transition-all duration-300 hover:scale-105 border border-[#D4B46A]/30 shadow-sm"
                  style={{ fontFamily: 'Inter, sans-serif' }}
                >
                  Clear Cart
                </button>
              ...
              {/* Always show checkout button for testing */}
              <button
                onClick={() => {
                  console.log('🛒 Checkout button clicked!')
                  console.log('📦 Items in cart:', items)
                  console.log('🧭 About to navigate to /checkout')
                  onClose()
                  navigate('/checkout')
                }}
                className="w-full bg-gradient-to-r from-[#0F2B1E] to-[#0A2418] hover:from-[#0A2418] hover:to-[#0F2B1E] text-[#F5F2E8] font-bold px-6 py-3 rounded-full transition-all duration-300 hover:scale-105 shadow-lg flex items-center justify-center gap-3"
                style={{ fontFamily: 'Playfair Display, serif' }}
              >
                <CreditCard className="w-5 h-5" />
                {items.length > 0 ? 'Secure Checkout' : 'Go to Checkout (Test)'}
