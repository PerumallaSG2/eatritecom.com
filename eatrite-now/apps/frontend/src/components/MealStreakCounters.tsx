...
export const MealStreakCounters: React.FC = () => {
  ...
  const getNextMilestone = () => {
    return streakMilestones.find(m => !m.isAchieved && m.days > streakData.currentStreak);
  };
      ...
      <div className="p-8">
        ...
        <FadeIn delay={0.1}>
          <div className="bg-white rounded-2xl p-8 mb-8 shadow-xl text-center">
            ...
            <div className={`inline-flex items-center space-x-2 px-4 py-2 rounded-full ${getStreakColor()}`}>
              <CheckCircle className="w-5 h-5" />
              <span className="font-semibold">
                {streakStatus === 'active' ? 'Streak Active!' : 
                 streakStatus === 'at-risk' ? 'Streak at Risk!' : 'Streak Broken'}
              </span>
            </div>
          </div>
        </FadeIn>
        ...
        <FadeIn delay={0.2}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white rounded-xl p-6 text-center shadow-lg">
              ...
              <div className="text-2xl font-bold text-gray-900">{streakData.totalDays}</div>
            ...
            <div className="bg-white rounded-xl p-6 text-center shadow-lg">
              ...
              <div className="text-2xl font-bold text-gray-900">
                {Math.round((streakData.currentStreak / streakData.totalDays) * 100)}%
              </div>
              ...
              {nextMilestone ? (
                <div>
                  <div className="flex items-center space-x-4 mb-4">
                    <div className={`w-12 h-12 bg-${nextMilestone.color}-100 rounded-xl flex items-center justify-center`}>
                      <nextMilestone.icon className={`w-6 h-6 text-${nextMilestone.color}-600`} />
                    </div>
                    <div>
                      <h4 className="font-bold text-gray-900">{nextMilestone.title}</h4>
                      <p className="text-sm text-gray-600">{nextMilestone.days} day streak</p>
                    </div>
                  </div>
                  ...
                  <div className={`p-3 bg-${nextMilestone.color}-50 rounded-lg`}>
                    <div className="flex items-center space-x-2">
                      <Star className={`w-4 h-4 text-${nextMilestone.color}-600`} />
                      <span className={`text-sm font-semibold text-${nextMilestone.color}-700`}>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2">
                          <h4 className={`font-semibold ${
                            milestone.isAchieved ? 'text-gray-900' : 'text-gray-500'
                          }`}>
                            {milestone.title}
                          </h4>
                          {milestone.isAchieved && (
                        ...
                        <p className="text-sm text-gray-600">{milestone.days} day milestone</p>
                        <p className={`text-xs ${
                          milestone.isAchieved 
                      ...
                      <div className="text-right">
                        <div className={`text-lg font-bold ${
                          milestone.isAchieved ? `text-${milestone.color}-600` : 'text-gray-400'
                        }`}>
                          {milestone.days}
                        </div>
        ...
        <FadeIn delay={0.6}>
          <div className="mt-8 bg-gradient-to-r from-orange-100 to-red-100 rounded-2xl p-8 text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">
              {streakStatus === 'active' ? 'Keep the Fire Burning!' : 'Don\'t Let the Streak Die!'}
            </h3>
            ...
            <button className="bg-gradient-to-r from-orange-600 to-red-600 text-white px-8 py-3 rounded-xl font-semibold hover:from-orange-700 hover:to-red-700 transition-all shadow-lg">
              {streakStatus === 'active' ? 'Order Next Meal' : 'Save My Streak!'}
            </button>
          </div>
        </FadeIn>
      </div>
