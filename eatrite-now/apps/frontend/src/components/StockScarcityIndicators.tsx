...
interface ScarcityAlert {
  id: string;
  type: 'low_stock' | 'high_demand' | 'last_chance' | 'selling_fast' | 'back_soon';
...
export const StockScarcityIndicators: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'stock' | 'demand' | 'popularity'>('stock');
    ...
    const activityTimer = setInterval(() => {
      setPurchaseActivity(prev => {
        const newActivity = {
          ...
          customerName: ['Alex S.', 'Jordan M.', 'Casey L.', 'Riley P.'][Math.floor(Math.random() * 4)],
          location: ['New York', 'Chicago', 'Austin', 'Miami'][Math.floor(Math.random() * 4)],
  ...
  const getDemandTrendIcon = (trend: string) => {
    switch (trend) {
      case 'surging': return <TrendingUp className="w-4 h-4 text-red-600" />;
      case 'increasing': return <TrendingUp className="w-4 h-4 text-orange-600" />;
      case 'stable': return <Activity className="w-4 h-4 text-blue-600" />;
      case 'decreasing': return <TrendingUp className="w-4 h-4 text-gray-600 transform rotate-180" />;
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center space-x-3 mb-2">
                ...
                <h2 className="text-2xl font-bold">Stock Scarcity Indicators</h2>
                <Flame className="w-6 h-6" />
              </div>
              <p className="text-yellow-100">
                Real-time stock levels and demand insights to create purchase urgency
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
              <div className="bg-white/10 rounded-lg p-3">
                <div className="text-2xl font-bold">
                  {stockItems.filter(item => item.stockLevel === 'critical' || item.stockLevel === 'low').length}
                </div>
                <div className="text-sm text-yellow-100">Low Stock Items</div>
              </div>
              <div className="bg-white/10 rounded-lg p-3">
                <div className="text-2xl font-bold">
                  {stockItems.filter(item => item.demandTrend === 'surging' || item.demandTrend === 'increasing').length}
                </div>
              ...
              <StaggeredAnimation>
                {scarcityAlerts.map((alert) => (
                  <Floating key={alert.id}>
                    <div className={`p-4 rounded-lg border-2 ${getAlertSeverityColor(alert.severity)} animate-pulse`}>
                      <div className="flex items-center space-x-2 mb-2">
                        {alert.severity === 'urgent' && <AlertTriangle className="w-5 h-5" />}
                        {alert.severity === 'warning' && <AlertCircle className="w-5 h-5" />}
                        {alert.severity === 'info' && <Info className="w-5 h-5" />}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <StaggeredAnimation>
                  {filteredItems.map((item) => {
                    ...
                    return (
                      <div key={item.id} className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all border border-gray-200">
                        {/* Item Header */}
                          ...
                          <div className="flex items-start space-x-3">
                            ...
                            <div>
                              <h4 className="font-bold text-gray-900 text-lg flex items-center">
                                {item.name}
                                {item.isTrending && <TrendingUp className="w-4 h-4 text-orange-500 ml-1" />}
                                {item.isNewArrival && <Sparkles className="w-4 h-4 text-blue-500 ml-1" />}
                                {item.isLimitedEdition && <Crown className="w-4 h-4 text-purple-500 ml-1" />}
                              </h4>
                              <div className="flex items-center space-x-2 text-sm text-gray-600">
                                <Star className="w-3 h-3 text-yellow-500" />
                                <span>{item.avgRating} ({item.totalReviews})</span>
                                <span>•</span>
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className={`text-xs px-2 py-1 rounded-full font-semibold ${
                                item.stockLevel === 'critical' ? 'bg-red-100 text-red-700' :
                                item.stockLevel === 'low' ? 'bg-orange-100 text-orange-700' :
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-4">
                          <div className="text-center p-2 bg-gray-50 rounded-lg">
                            ...
                            <div className="font-semibold text-blue-600">{item.viewsLast24h.toLocaleString()}</div>
                          </div>
                          ...
                          <div className="text-center p-2 bg-gray-50 rounded-lg">
                            ...
                            <div className="font-semibold text-orange-600">{item.addedToCartLast24h}</div>
                          </div>
                          ...
                          <div className="text-center p-2 bg-gray-50 rounded-lg">
                            ...
                            <div className="font-semibold text-green-600">{item.purchasesLast24h}</div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs font-semibold capitalize">{item.demandTrend}</span>
                          </div>
                        </div>
                        ...
                        {item.stockLevel === 'critical' && (
                          <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                            <div className="flex items-center space-x-2 text-red-700">
                              ...
                              <span className="font-semibold text-sm">High Demand Alert!</span>
                            </div>
                            <p className="text-xs text-red-600 mt-1">
                              This item is selling fast. Only {item.currentStock} remaining.
                            </p>
                          </div>
                        )}
                        ...
                        {item.demandTrend === 'surging' && (
                          <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
                            ...
                            <p className="text-xs text-orange-600 mt-1">
                              {item.purchasesLast24h} people bought this in the last 24 hours.
                            </p>
                          </div>
                        )}
                        ...
                        <button className={`w-full py-3 rounded-lg font-semibold transition-colors ${
                          ...
                          {item.stockLevel === 'critical' ? 'Order Now - Limited Stock!' : 'Add to Cart'}
                        </button>
                      </div>
                    );
                  })}
                    ...
                    <div key={activity.id} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        <div className="text-sm">
                          ...
                          {activity.isVerified && <Badge className="w-3 h-3 text-blue-500 inline ml-1" />}
                        </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-green-600">
                      {purchaseActivity.filter(a => a.timeAgo.includes('minute')).length + 47}
                    </div>
                    <div className="text-sm text-gray-600">Orders in last hour</div>
                  </div>
                </div>
              </div>
            </FadeIn>
