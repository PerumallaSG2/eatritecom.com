...
const Skeleton: React.FC<SkeletonProps> = ({ 
  ...
  const baseClasses = 'bg-gradient-to-r from-[#F5EEDC] via-[#D4B46A]/20 to-[#F5EEDC] rounded-lg';
...
export const MealCardSkeleton: React.FC<MealCardSkeletonProps> = ({ className = '' }) => {
  return (
    <div className={`bg-white rounded-2xl shadow-2xl overflow-hidden animate-pulse ${className}`}>
      {/* Image Skeleton */}
      ...
      <div className="p-8 space-y-6">
        ...
        {/* Nutrition & Button */}
        <div className="flex items-center justify-between">
          <Skeleton variant="rectangular" width="120px" height="32px" className="rounded-full" />
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0F2B1E] to-[#0A2418]">
      <div className="text-center space-y-8">
        ...
        <div className="flex justify-center space-x-2">
          <div className="w-2 h-2 bg-[#D4B46A] rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></div>
          <div className="w-2 h-2 bg-[#D4B46A] rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></div>
          <div className="w-2 h-2 bg-[#D4B46A] rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></div>
        </div>
      </div>
    </div>
  );
