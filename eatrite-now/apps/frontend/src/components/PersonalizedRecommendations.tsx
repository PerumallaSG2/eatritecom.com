...
> = ({ meals, onAddToCart, onToggleFavorite }) => {
  const {
    preferences,
  ...
  const getRecommendationReason = (meal: any) => {
    ...
    const matchingRestrictions = dietaryProfile.restrictions.filter(
      restriction => mealTags.includes(restriction.toLowerCase())
    )
    ...
    const calorieMatch =
      Math.abs(meal.calories - dietaryProfile.calorieGoal) < 100
    if (calorieMatch) {
      return {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Personalized based on your dietary profile
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {recommendedMeals.map((meal, index) => {
          const isFavorite =
            preferences?.favoriteMeals.includes(meal.id) || false
              ...
              {index < 3 && (
                <div className="absolute top-3 left-3 z-10">
                  <div className="bg-gradient-to-r from-green-500 to-green-600 text-white px-2 py-1 rounded-full text-xs font-bold flex items-center shadow-lg">
                    <Star className="w-3 h-3 mr-1 fill-current" />#{index + 1}{' '}
                    Pick
                  </div>
                </div>
              )}
              ...
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                {reason && (
                  <div className="absolute bottom-3 left-3 right-3">
                    <div
                      className={`flex items-center px-3 py-2 rounded-full text-xs font-medium ${reason.color} backdrop-blur-sm`}
                    >
                      {reason.icon}
