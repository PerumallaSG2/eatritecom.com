  ...
  const partnershipBenefits = [
    {
      ...
      description: 'Improve employee health and satisfaction with premium meal options',
      icon: Award,
      metrics: ['94% satisfaction', '23% health improvement', '15% productivity boost']
    },
    {
      title: 'Cost Savings',
      description: 'Reduce corporate food costs while providing better nutrition',
      icon: DollarSign,
      metrics: ['30% cost reduction', 'Bulk pricing available', 'Tax-deductible wellness benefit']
    },
    {
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-12">
            <div className="mb-8 lg:mb-0">
              ...
              <p className="text-xl text-white/90 max-w-2xl leading-relaxed">
                Track ROI, monitor partnerships, and optimize corporate wellness programs 
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                {timeframes.map((tf) => (
                  <option key={tf.id} value={tf.id} className="text-gray-800">{tf.label}</option>
                ))}
              </select>
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                    selectedMetric === metric.id ? 'bg-[#D4B46A]/20' : 'bg-gray-50'
                  }`}>
                    <metric.icon className={`w-6 h-6 ${
                      selectedMetric === metric.id ? 'text-[#D4B46A]' : 'text-gray-600'
                    }`} />
                  </div>
                ...
                <div className="mb-4">
                  <div className="text-3xl font-bold text-[#0F2B1E] mb-2">{metric.value}</div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
            <div>
              <h2 className="text-3xl font-semibold text-[#0F2B1E] font-playfair">Recent Partnerships</h2>
              <p className="text-gray-600 mt-2">Latest corporate partnerships and their performance metrics</p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#F5EEDC]">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#0F2B1E]">Company</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#0F2B1E]">Industry</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#0F2B1E]">Employees</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#0F2B1E]">Monthly Value</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#0F2B1E]">Start Date</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#0F2B1E]">Status</th>
                  </tr>
                </thead>
                    ...
                    <tr key={partnership.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          ...
                          <div className="font-semibold text-[#0F2B1E]">{partnership.company}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-gray-700 font-medium">{partnership.employees.toLocaleString()}</td>
                      <td className="px-6 py-4 font-bold text-[#D4B46A]">{partnership.monthlyValue}</td>
                      <td className="px-6 py-4 text-gray-600">{new Date(partnership.startDate).toLocaleDateString()}</td>
                      <td className="px-6 py-4">
      ...
      <section className="py-16 bg-[#F5EEDC]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div>
              <h3 className="text-2xl font-semibold text-[#0F2B1E] mb-6 font-playfair">Partnership by Industry</h3>
              <div className="bg-white rounded-xl border border-gray-200 p-8">
                  ...
                  {industryBreakdown.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="font-bold text-[#0F2B1E] text-lg">{item.partners}</div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-200">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-bold text-[#0F2B1E]">156 Companies</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-2xl font-semibold text-[#0F2B1E] mb-6 font-playfair">Partnership Benefits</h3>
              <div className="space-y-6">
                {partnershipBenefits.map((benefit, idx) => (
                  <div key={idx} className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-shadow duration-200">
                    <div className="flex items-start">
                      ...
                      <div className="flex-1">
                        <h4 className="text-lg font-semibold text-[#0F2B1E] mb-2">{benefit.title}</h4>
                        <p className="text-gray-600 mb-4 leading-relaxed">{benefit.description}</p>
                        <div className="flex flex-wrap gap-2">
                          {benefit.metrics.map((metric, metricIdx) => (
                            <span key={metricIdx} className="bg-green-50 text-green-700 px-3 py-2 rounded-lg text-sm font-medium border border-green-200">
                              {metric}
                            </span>
                          ))}
                        </div>
      ...
      <section className="py-20 bg-gradient-to-br from-[#0F2B1E] to-[#1a4d33] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-white/90 mb-12 max-w-3xl mx-auto leading-relaxed">
            Join 150+ companies providing premium wellness benefits to their employees. 
            Let's discuss how we can customize a solution for your organization.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl mx-auto">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
              ...
              <div className="font-semibold text-white mb-2">Call Partnership Team</div>
              <div className="text-white/80 mb-2">1-800-EATRITE</div>
              <div className="text-sm text-white/60">Available Mon-Fri 9AM-6PM EST</div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
              ...
              <div className="font-semibold text-white mb-2">Email Partnerships</div>
              <div className="text-white/80 mb-2">partnerships@eatrite.com</div>
              <div className="text-sm text-white/60">24-hour response guarantee</div>
            </div>
          </div>
        </div>
      </section>
