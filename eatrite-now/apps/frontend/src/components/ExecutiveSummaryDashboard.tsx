...
const generateSalesData = (): SalesData[] => [
  { period: 'Q1 2024', revenue: 7200000, orders: 92000, avgOrderValue: 78.26, newCustomers: 18500, retentionRate: 84.2 },
  { period: 'Q2 2024', revenue: 8100000, orders: 98500, avgOrderValue: 82.23, newCustomers: 21200, retentionRate: 86.7 },
  { period: 'Q3 2024', revenue: 8650000, orders: 103200, avgOrderValue: 83.85, newCustomers: 19800, retentionRate: 88.1 },
  { period: 'Q4 2024', revenue: 9200000, orders: 108700, avgOrderValue: 84.65, newCustomers: 22100, retentionRate: 89.3 }
];
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h2 className="text-3xl font-bold">Executive Summary Dashboard</h2>
                    ...
                    {metric.benchmark && (
                      <div className="flex items-center justify-between text-sm text-gray-500 mb-3">
                        ...
                        <span className="font-semibold">
                          {metric.unit === '$' ? formatCurrency(metric.benchmark.value) : 
          ...
          <FadeIn delay={0.2}>
            <div className="bg-white rounded-2xl p-6 shadow-lg">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-sm text-gray-500">{selectedTimeframe} view</span>
              </div>
              ...
              <div className="space-y-4">
                {salesData.map((data, index) => {
                  ...
                  return (
                    <div key={data.period} className={`p-4 rounded-lg ${isLatest ? 'bg-blue-50 border border-blue-200' : 'bg-gray-50'}`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className={`font-semibold ${isLatest ? 'text-blue-900' : 'text-gray-900'}`}>
                          {data.period}
                        </span>
                        <span className={`text-sm ${isLatest ? 'text-blue-600' : 'text-gray-600'}`}>
                      ...
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <div className="font-semibold">{formatCurrency(data.avgOrderValue)}</div>
                        </div>
                        <div>
                          <div className="text-gray-600">Retention</div>
                          <div className="font-semibold">{data.retentionRate}%</div>
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg">
              ...
              <div className="space-y-4">
                {healthImpacts.map((impact) => (
                  <div key={impact.id} className="p-4 bg-gradient-to-r from-red-50 to-pink-50 rounded-lg border border-red-100">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-2">
                        <span className="text-2xl">{getHealthCategoryIcon(impact.category)}</span>
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-gray-600">{impact.timeframe}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-green-600 font-semibold">+{impact.improvement}% improvement</span>
                      </div>
                      <div className="flex items-center space-x-1 text-gray-600">
                        <Star className="w-3 h-3" />
                        <span>{formatNumber(impact.testimonials)} testimonials</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-4 p-4 bg-gradient-to-r from-green-50 to-blue-50 rounded-lg border border-green-200">
                <div className="text-center">
                  ...
                  <div className="text-sm text-gray-700">Customer Health Goal Achievement Rate</div>
                  <div className="text-xs text-gray-600 mt-1">Highest in the industry</div>
                </div>
              </div>
            </div>
          </FadeIn>
                ...
                {roiMetrics.map((roi, index) => (
                  <div key={index} className="p-4 bg-purple-50 rounded-lg border border-purple-100">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-semibold text-gray-900">{roi.category}</h4>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-3">
                      <div>
                        ...
                        <div className="font-semibold text-red-600">{formatCurrency(roi.investment)}</div>
                      </div>
                      <div>
                        <div className="text-sm text-gray-600">Return</div>
                        <div className="font-semibold text-green-600">{formatCurrency(roi.return)}</div>
                      </div>
                    </div>
                  ...
                  <div key={index} className="p-4 bg-indigo-50 rounded-lg border border-indigo-100">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="font-semibold text-gray-900">{partner.partner}</h4>
                      ...
                      <div className="text-right">
                        <div className="font-semibold text-indigo-600">{formatCurrency(partner.revenue)}</div>
                        <div className="text-xs text-gray-600">{formatNumber(partner.customers)} customers</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="text-indigo-600 font-semibold">
                        ${Math.round(partner.revenue / partner.customers)} per customer
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-4 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg border border-indigo-200">
                <div className="text-center">
                  <div className="text-2xl font-bold text-indigo-600 mb-1">
                    {formatCurrency(partnerships.reduce((sum, p) => sum + p.revenue, 0))}
                  </div>
                  <div className="text-sm text-gray-700">Total Partnership Revenue</div>
                  <div className="text-xs text-gray-600 mt-1">
                    {formatNumber(partnerships.reduce((sum, p) => sum + p.customers, 0))} total B2B customers
                  </div>
                </div>
              </div>
            </div>
