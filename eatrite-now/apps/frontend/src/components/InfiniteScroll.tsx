...
function InfiniteScroll<T>({
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (target.isIntersecting && !loading && (displayedItems.length < items.length || hasMore)) {
          loadMoreItems();
        }
      },
      {
  ...
  }, [loadMoreItems, loading, displayedItems.length, items.length, hasMore, threshold]);
    ...
    <div className={className}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
        {displayedItems.map((item, index) => (
          <div key={`item-${index}`} className="transition-opacity duration-300">
      ...
      >
        {loading ? (
          <div className="flex items-center space-x-2 text-green-600">
            ...
            <span className="text-sm font-medium">Loading more delicious meals...</span>
          </div>
