      ...
      .then((cacheNames) => {
        const deletePromises = cacheNames
          .filter((cacheName) => {
            ...
            return cacheName.startsWith('eatrite-') && 
...
async function handleRequest(request) {
  ...
  try {
    ...
    if (CACHE_FIRST.some(path => pathname.startsWith(path) || url.origin.includes(path.replace('/', '')))) {
...
async function staleWhileRevalidate(request) {
  ...
  return cachedResponse || await networkPromise || createOfflineResponse(request);
}
...
async function isCacheFresh(request, cachedResponse) {
  ...
  } else if (url.pathname.includes('/images/') || url.hostname.includes('unsplash')) {
